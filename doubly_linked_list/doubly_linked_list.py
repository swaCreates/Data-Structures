...
class DoublyLinkedList:
    ...
    def move_to_front(self, node):
        if node is self.head:
            return # if node is in the front already just return out of fn
        old_val = node.value
        self.delete(node)
        self.add_to_head(old_val)
    ...
    def get_max(self):
        #  if no head means no list
        if not self.head:
            return None
        # getting value of head
        max_val = self.head.value
        current = self.head
        while current:
            if current.value > max_val:
                #  if current node val is greater than previous node make that the max val
